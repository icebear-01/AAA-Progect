...
### Sanity check (Neural A* single run)
```sh
(.venv) $ python scripts/sanity_neural_astar_run.py
```

This runs one random 64x64 map and prints:
- `found_path`
- `path_length`
- `expanded_nodes`
- `runtime_ms`
...
## Route A Integration (Guidance + Hybrid A*)

This repository now includes a minimal Route A path:
- Neural encoder predicts a 2D guidance `cost_map` from `(occ_map, start_map, goal_map)`.
- Hybrid A* remains the kinematic-feasible planner.
- Integration term at expansion:
  `g_total = g_kinematic + lambda_guidance * cost_map[y, x]`

### Coordinate convention (enforced in new code)
- World/state node coordinates: `(x, y)`
- Numpy/torch indexing: `[y, x]`
- Occupancy semantics for Route A data: `occ_map=1` obstacle, `0` free
- Cost-map semantics: lower is better; obstacle cells are masked to high cost.

### Phase 1: Dummy parking dataset generation
Generate temporary expert data with 2D 8-connected A*:
```sh
(.venv) $ python tools/make_dummy_parking_npz.py \
  --num-samples 200 \
  --size 64 \
  --out-dir outputs/dummy_parking/train
```

Inspect one sample:
```sh
(.venv) $ python scripts/inspect_dataset.py \
  --data-dir outputs/dummy_parking/train \
  --index 0 \
  --output outputs/dummy_parking/sample0.png
```

### Phase 2-3: Train guidance encoder
```sh
(.venv) $ python scripts/train_guidance_encoder.py \
  --train-dir outputs/dummy_parking/train \
  --epochs 10 \
  --batch-size 32 \
  --device cpu \
  --output-dir model/guidance_encoder
```

Optional validation A* metric:
```sh
(.venv) $ python scripts/train_guidance_encoder.py \
  --train-dir outputs/dummy_parking/train \
  --epochs 10 \
  --compute-grid-astar-metric \
  --astar-lambda 1.0
```

### Phase 4: Inference API for Hybrid A* guidance
Python API:
```python
from neural_astar.api.guidance_infer import infer_cost_map

cost = infer_cost_map(
    ckpt_path="model/guidance_encoder/best.pt",
    occ_map_numpy=occ_map,     # [H, W], 1=obstacle
    start_xy=(sx, sy),
    goal_xy=(gx, gy),
    device="cpu",
)
...
CLI:
```sh
(.venv) $ python -m neural_astar.api.guidance_infer \
  --ckpt model/guidance_encoder/best.pt \
  --occ_npz outputs/dummy_parking/train/sample_00000.npz \
  --start 2 2 \
  --goal 50 50 \
  --out outputs/cost_map.npy
```

### Phase 5: Guided Hybrid A* demo evaluation
```sh
(.venv) $ python scripts/eval_guided_hybrid_astar_demo.py \
  --num-problems 20 \
  --lambda-guidance 2.0
```

Strict terminal pose with RS-shot (no goal tolerance):
```sh
(.venv) $ python scripts/eval_guided_hybrid_astar_demo.py \
  --num-problems 20 \
  --strict-goal-pose \
  --goal-tolerance-yaw-deg 0 \
  --lambda-guidance 2.0
```

With learned guidance:
```sh
(.venv) $ python scripts/eval_guided_hybrid_astar_demo.py \
  --num-problems 20 \
  --ckpt model/guidance_encoder/best.pt \
...
### Phase 6: Expert dataset generation hook (real Hybrid A*)
```sh
(.venv) $ python tools/generate_expert_dataset_from_hybrid_astar.py \
  --num-maps 50 \
  --pairs-per-map 4 \
  --out-dir outputs/expert_hybrid
```

Edit `run_hybrid_astar(...)` in
`tools/generate_expert_dataset_from_hybrid_astar.py`
to call your own Hybrid A* implementation.

### Phase 7: RL fine-tuning scaffold (optional)
Scaffold files are in `rl_finetune/`.
```sh
(.venv) $ python rl_finetune/train_ppo.py --ckpt model/guidance_encoder/best.pt
```
